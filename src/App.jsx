...
function App() {
    ...
    const GEMINI_MODELS = ['gemini-2.5-flash', 'gemini-2.5-flash'];
    ...
    return (
        <div className="bg-slate-100 min-h-screen font-sans text-slate-900 flex flex-col items-center justify-start p-4 sm:p-6 lg:p-8">
            ...
            <main className="w-full max-w-2xl mx-auto">
                ...
                {view === 'analysis' && (
                    <div className="bg-white rounded-2xl shadow-xl w-full p-6 sm:p-8">
                        <div className="relative w-full h-64 sm:h-96 mb-6 rounded-xl overflow-hidden bg-slate-50 border border-slate-200">
                            <img src={image} alt="Selected fruit" className="absolute inset-0 w-full h-full object-contain p-2" />
                        </div>
...
const AnalysisTabs = ({ analysisResult, onLogFruit, apiKey }) => {
    ...
    const generateRecipeImage = async () => {
        ...
        try {
            // Extract the main recipe name from the recipe idea
